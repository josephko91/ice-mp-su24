...
def get_environmental_vars(nc, trajs, selected_vars):
    ...
    trajs['time index'] = t_index_array

    for var in selected_vars:
        values = []
        for tindex, zk, yj, xi in zip(t_index_array, trajs['zh gridbox'].values, trajs['yk gridbox'].values, trajs['xi gridbox'].values):
            value = nc[var][tindex, zk, yj, xi].values * 1
            values.append(value)
        trajs[var] = values
